...
class CBallGridData(
    ...
    private fun findPath1(source: Point, target: Point): Boolean {
        if ((source.x !in 0..<rowCounts) ||
            (source.y !in 0..<colCounts)) {
            // excess the range
            return false
        }
        if ((target.x !in 0..<rowCounts) ||
            (target.y !in 0..<colCounts)) {
            // excess the range
            return false
        }
